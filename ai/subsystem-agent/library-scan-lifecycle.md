...
- `Test Files` checks source existence only. It marks missing sources dead and hides them from views without listing archives, materializing entries, or reading metadata.
...
- Scan progress is throttled and terminal operation state clears the job before late telemetry can revive Cancel.
...
- Scan scratch roots use the exact `spcboy-scan-scratch-*` prefix. They are disposable, never a playback cache, and are removed after normal completion, cancellation, or materialization failure.
